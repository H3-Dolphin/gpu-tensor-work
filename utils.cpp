...
void createGaussianKernel(double* weights, int* space_ofs, int& maxk, const int radiusH, const int radiusV, const double sigma, const int imstep, const bool isRectangle)
{
	maxk = 0;
	...
	for (int i = -radiusV; i <= radiusV; i++)
	{
		for (int j = -radiusH; j <= radiusH; j++)
		{
			...
			if (r > maxr && !isRectangle)
				continue;
			weights[maxk] = exp(static_cast<double>(i * i + j * j) / (-2.f * sigma * sigma));
			space_ofs[maxk++] = (int)(i * imstep + j);
		}
	}
}

void createGaussianKernel(double* weights, int* space_ofs, int& maxk, const int radius, const double sigma, const int imstep, const bool isRectangle)
{
	createGaussianKernel(weights, space_ofs, maxk, radius, radius, sigma, imstep, isRectangle);
}


void createGaussianKernel(float* weights, int* space_ofs, int& maxk, const int radiusH, const int radiusV, const float sigma, const int imstep, const bool isRectangle)
{
	maxk = 0;
	const int maxr = std::max(radiusV, radiusH);
	for (int i = -radiusV; i <= radiusV; i++)
	{
		for (int j = -radiusH; j <= radiusH; j++)
		{
			const double r = std::sqrt(static_cast<float>(i * i + j * j));
			if (r > maxr && !isRectangle)
				continue;
			weights[maxk] = exp(static_cast<float>(i * i + j * j) / (-2.f * sigma * sigma));
			space_ofs[maxk++] = (int)(i * imstep + j);
		}
	}
}

void createGaussianKernel(float* weights, int* space_ofs, int& maxk, const int radius, const float sigma, const int imstep, const bool isRectangle)
{
	createGaussianKernel(weights, space_ofs, maxk, radius, radius, sigma, imstep, isRectangle);
}

void createGaussianKernel(half* weights, int* space_ofs, int& maxk, const int radiusH, const int radiusV, const float sigma, const int imstep, const bool isRectangle)
{
	maxk = 0;
	const int maxr = std::max(radiusV, radiusH);
	for (int i = -radiusV; i <= radiusV; i++)
	{
		for (int j = -radiusH; j <= radiusH; j++)
		{
			...
			if (r > maxr && !isRectangle)
				continue;
			weights[maxk] = __float2half(exp(static_cast<float>(i * i + j * j) / (-2.f * sigma * sigma)));
			space_ofs[maxk++] = (int)(i * imstep + j);
		}
	}
}

void createGaussianKernel(half* weights, int* space_ofs, int& maxk, const int radius, const float sigma, const int imstep, const bool isRectangle)
{
	createGaussianKernel(weights, space_ofs, maxk, radius, radius, sigma, imstep, isRectangle);
}
